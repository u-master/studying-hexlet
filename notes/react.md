...
Позволяют использовать в React только функциональные компоненты.

## useState

Позволяет использовать в функциональной компоненте локальное состояние.

    import { useState } from 'react';
    
Как результат действия функции useState, мы получаем массив, состоящий из двух элементов: текущее состояние и функция для изменения состояния.
На вход мы подаем начальное значение состояния (т.е. если состояние еще не было задано, это начальное возвращается как текущее состояние).
Можно использовать для каждого стейта свой хук.

    export default TodoApp () => {
      const defaultNewTodo = 0;
      const defaultTodos = [
          { id: 0, task: 'Buy milk', completed: false },
          { id: 1, task: 'Make work', completed: false },
      ];
      const [newTodo, setNewTodo] = useState(defaultNewTodo);
      const [todos, setTodos] = useState(defaultTodos);
      const addTodo = e => {
        if (e.key === 'Enter') {
          setTodos([...todos, { id: uniqueId(), task: newTodo, completed: false }]);
          setNewTodo('');
        }
      }
      return (
...
Эта функция позволяет реализовывать lifecycle хуки компонента (хуки жизненного цикла компонента).
Позволяет заменить, такие колбеки, как `componentDidMount`, `componentDidUpdate` и `componentWillUnmount`.

    import { useEffect } from 'react';
    
Первым параметром мы передаем колбек, который будет выполняться при наступлении события, а
вторым параметром передается массив зависимостей, на котрые будет откликаться useEffect.
Колбек может возвращать функцию, и эта функция эквивалентна выполнению `componentWillUnmount`!
    
    useEffect(() => { 
      /* do smth */
      return /* do smth --> componentWillUnmount */ ;
    }, []); // [] --> componentDidMount
    
    useEffect(() => { /* do smth */ }, [newTodo]); // хук выполняется, когда изменяется стейт newTodo
